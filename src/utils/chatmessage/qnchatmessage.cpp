...
QNChatMessage::QNChatMessage(QPixmap myImage,QPixmap friendImage,QWidget *parent) : QWidget(parent)
{
...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    // 调整界面固有元素
    m_leftPixmap = friendImage;
    m_rightPixmap = myImage;

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/loding/img/loading4.gif");
...
QSize QNChatMessage::getRealString(QString src)
{
    ...
    if(nCount == 0) {
        ...
        if(nMaxWidth > m_textWidth) {
            ...
            int size = m_textWidth / fm.boundingRect(" ").width();
            int num = fm.boundingRect(value).width() / m_textWidth;
            int ttmp = num*fm.boundingRect(" ").width();
        ...
        for(int i = 0; i < (nCount + 1); i++) {
            ...
            nMaxWidth = fm.boundingRect(value).width() > nMaxWidth ? fm.boundingRect(value).width() : nMaxWidth;
    ...
    return QSize(nMaxWidth+m_spaceWid+2, (nCount + 1) * m_lineHeight+2*m_lineHeight);
}
